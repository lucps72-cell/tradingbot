        ...
        try:
            ...
            for position_symbol in sorted(active_positions.keys()):
                ...
                if long_position['side'] == 'long' and long_amount > 0:
                    logger.info(f"{Colors.GREEN}[포지션 상태] {Colors.BOLD}{position_symbol} LONG{Colors.RESET} : 진입가= {long_entry:.4f}, 수량= {long_amount}, 손절가= {long_sl:.4f}, 익절가= {long_tp:.4f}, 손익률= {long_pnl_rate}%, 손익액= {long_pnl_amount}{Colors.END}")
                if short_position['side'] == 'short' and short_amount > 0:
                    logger.info(f"{Colors.RED}[포지션 상태] {Colors.BOLD}{position_symbol} SHORT{Colors.RESET} : 진입가= {short_entry:.4f}, 수량= {short_amount}, 손절가= {short_sl:.4f}, 익절가= {short_tp:.4f}, 손익률= {short_pnl_rate}%, 손익액= {short_pnl_amount}{Colors.END}")
