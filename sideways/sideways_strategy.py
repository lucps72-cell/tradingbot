...
class SidewaysStrategy:
    ...
    def __init__(self, exchange, symbol, config, window_size=180+1, bb_window=20, bb_std=2, rsi_period=14, rsi_overbought=70, rsi_oversold=30):
        ...
        # 추세감지 결과 리스트 저장용
        self.result_first_trend = None
        self.result_second_trend = None
        self.pre_first_ema_up_key = None
        self.pre_first_ema_down_key = None
        self.pre_second_ema_up_key = None
        self.pre_second_ema_down_key = None
    ...
    def execute_trading(self, current_trend, verbose: bool = False):
        ...
        # 1차 추세 판단(5분봉)
        self.result_first_trend, check_msg = self.determine_trend_signal(
        ...
        # 2차 추세 판단(1분봉)
        self.result_second_trend, check_msg = self.determine_trend_signal(
        ...
        # 진입제한(상승/하락추세 시 반대진입 제한)
        if ema_hgh_up and ema_low_up:
            if (return_position_hgh == "short" or return_position_low == "short"):
                if (return_msg_hgh and "⭐️" not in return_msg_hgh[:1]) or (return_msg_low and "⭐️" not in return_msg_low[:1]):
                    return_position_hgh = None
                    return_position_low = None
                    active_logger.info(f"❌ 진입 거절(상승추세) : 매도 | price: {current_price} | {return_position_msg}")
                    trade_logger.info(f"❌ 진입 거절(상승추세) : 매도 | price: {current_price} | {return_position_msg}")
        elif ema_hgh_down and ema_low_down:
           if (return_position_hgh == "long" or return_position_low == "long"):
                if (return_msg_hgh and "⭐️" not in return_msg_hgh[:1]) or (return_msg_low and "⭐️" not in return_msg_low[:1]):
                    return_position_hgh = None
                    return_position_low = None
                    active_logger.info(f"❌ 진입 거절(하락추세) : 매수 | price: {current_price} | {return_position_msg}")
                    trade_logger.info(f"❌ 진입 거절(하락추세) : 매수 | price: {current_price} | {return_position_msg}")
                       
        # 추세전환 골든크로스/데드크로스 신호
        if self.pre_first_ema_up_key != ema_hgh_up and ema_hgh_up is True and \
           self.pre_second_ema_up_key == ema_low_up and ema_low_up is True:
            return_position_low = "long"
            return_position_msg = f"신호 없음 : 매수 전환(상승시작 골든크로스) | {return_position_msg}"
        elif self.pre_first_ema_down_key != ema_hgh_down and ema_hgh_down is True and \
            self.pre_second_ema_down_key == ema_low_down and ema_low_down is True:
            return_position_low = "short"
            return_position_msg = f"신호 없음 : 매도 전환(하락시작 데드크로스) | {return_position_msg}"
    
        # 추세전환 (ema 이전값과 비교) : 추세가 횡보일때 ema값 전환으로 진입신호 감지
        if self.result_first_trend is None and self.result_second_trend is None and return_position_low is None and return_position_hgh is None:
            if self.pre_first_ema_up_key and self.pre_first_ema_up_key == self.pre_second_ema_up_key and ema_hgh_up and ema_low_up is False:
                return_position_low = "short"
                return_position_msg = f"신호 없음 : 매도 전환(하락시작) | {return_position_msg}"
            elif self.pre_first_ema_down_key and self.pre_first_ema_down_key == self.pre_second_ema_down_key and ema_hgh_down and ema_low_down is False:
                return_position_low = "long"
                return_position_msg = f"신호 없음 : 매수 전환(상승시작) | {return_position_msg}"
        self.pre_first_ema_up_key = ema_hgh_up
        self.pre_first_ema_down_key = ema_hgh_down
        self.pre_second_ema_up_key = ema_low_up
        self.pre_second_ema_down_key = ema_low_down        

        # 추세전환 - 매도 진입 (상승 후 하락 시작 시점)
        # 1차(5분): 직전 종가가 밴드 고점 위, 직전 RSI > 65, 현재가 < 직전 종가, RSI < 이전 RSI
        # 2차(1분): 직전 종가가 밴드 고점 위, 직전 RSI > 70, 현재가 < 직전 종가, RSI < 이전 RSI
        if return_position_hgh is None and return_position_low is None:
            # 1차(5분봉)
            if len(df_hgh) > 1 and len(rsi_hgh) > 1:
                ...
                if prev_close2 > prev_bb_up2 and prev_rsi2 > 70 and current_price < prev_close1:
                    return_position_hgh = "short"
                    return_position_msg = f"매도 진입(5m): 상승 후 하락 시작 신호 | {return_position_msg}"
                    active_logger.info(f"매도 진입(5m): 직전고가({prev_close2:.4f})>밴드고점({prev_bb_up2:.4f}), 직전RSI({prev_rsi2:.2f})>70, 현재가({current_price:.4f})")
            # 2차(1분봉)
            if len(df_low) > 1 and len(rsi_low) > 1:
                prev_close2 = df_low['close'].iloc[-2]
                prev_close1 = df_low['close'].iloc[-1]
                prev_rsi2 = rsi_low.iloc[-2]
                prev_rsi1 = rsi_low.iloc[-1]
                prev_bb_up2 = bb_low_top.iloc[-2]
                if prev_close2 > prev_bb_up2 and prev_rsi2 > 70 and prev_close1 < prev_close2 and current_price < prev_close1 and prev_rsi1 < prev_rsi2:
                    return_position_low = "short"
                    return_position_msg = f"매도 진입(1m): 상승 후 하락 시작 신호 | {return_position_msg}"
                    active_logger.info(f"매도 진입(1m): 직전종가({prev_close2:.4f})>밴드고점({prev_bb_up2:.4f}), 직전RSI({prev_rsi2:.2f})>70, 현재가({current_price:.4f})<직전종가, RSI하락({prev_rsi1:.2f}<{prev_rsi2:.2f})")

        # 추세전환 - 매수 진입 (하락 후 반등 시작 시점)
        # 1차(5분): 직전 저가 < 밴드 하단, 직전 종가 밴드 저점 위, 직전 RSI < 35, 현재가 > 직전 시가, RSI > 이전 RSI
        # 2차(1분): 직전 종가가 밴드 저점 위, 직전 RSI < 30, 현재가 > 직전 시가, RSI > 이전 RSI
        if return_position_hgh is None and return_position_low is None:
            # 1차(5분봉)
            if len(df_hgh) > 1 and len(rsi_hgh) > 1:
                ...
                if prev_close2 < prev_bb_dn2 and prev_rsi2 < 30 and current_price > prev_close1:
                    active_logger.info(f"매수 진입(5m): 직전종가({prev_close2:.4f})<밴드저점({prev_bb_dn2:.4f}), 직전RSI({prev_rsi2:.2f})<30, 현재가({current_price:.4f})>직전종가({prev_close1:.4f}), RSI상승({prev_rsi1:.2f}>{prev_rsi2:.2f})")
                    return_position_hgh = "long"
                    return_position_msg = f"매수 진입(5m): 하락 후 반등 시작 신호 | {return_position_msg}"
            # 2차(1분봉)
            if len(df_low) > 1 and len(rsi_low) > 1:
                prev_close2 = df_low['close'].iloc[-2]
                prev_close1 = df_low['close'].iloc[-1]
                ...
                if prev_close2 < prev_bb_dn2 and prev_rsi2 < 30  and prev_close1 > prev_close2 and current_price > prev_close1 and prev_rsi1 > prev_rsi2:
                    active_logger.info(f"매수 진입(1m): 직전종가({prev_close2:.4f})<밴드저점({prev_bb_dn2:.4f}), 직전RSI({prev_rsi2:.2f})<30, 현재가({current_price:.4f})>직전종가({prev_close1:.4f}), RSI상승({prev_rsi1:.2f}>{prev_rsi2:.2f})")
                    return_position_low = "long"
                    return_position_msg = f"매수 진입(1m): 하락 후 반등 시작 신호 | {return_position_msg}"

        # 되돌림 보완 체크(2차 추세)
        # if return_position_hgh is None and return_position_low is None:
        #     try:
        #         # 볼린저밴드 상단/하단 돌파 여부 계산
        #         h_bb_up = current_price > bb_low_top.iloc[-1] if hasattr(bb_low_top, 'iloc') else current_price > bb_low_top
        #         h_bb_down = current_price < bb_low_down.iloc[-1] if hasattr(bb_low_down, 'iloc') else current_price < bb_low_down
        #         # get_pullback_signal에 시계열 dict(ema_low_series)와 계산된 밴드 돌파 여부 전달
        #         pullback_signal = self.get_pullback_signal(
        #             ema_low_series, h_bb_up, h_bb_down, current_price, self.config, self.return_second_trend
        #         )
        #         if pullback_signal is not None and pullback_signal == 'long' and self.return_second_trend == 'uptrend':
        #             self.return_second_trend = "uptrend"
        #             active_logger.info(f"{Colors.GREEN}🔄 2차 추세 보완 : 상승 (추세 되돌림) : downtrend → uptrend {Colors.END}")
        #         elif pullback_signal is not None and pullback_signal == 'short' and self.return_second_trend == 'downtrend':
        #             self.return_second_trend = "downtrend"
        #             active_logger.info(f"{Colors.RED}🔄 2차 추세 보완 : 하락 (추세 되돌림) : uptrend → downtrend {Colors.END}")
        #         else:
        #             active_logger.info("2차 추세 보완 : 해당없음 (추세 되돌림)")
        #     except Exception as e:
        #         active_logger.error(f"2차 추세 보완 체크 오류: {str(e)}")


        # 최종 진입/청산 확인
        if return_position_hgh == "long" or return_position_low == "long":
            active_logger.info(f"{Colors.GREEN}🟢 진입 : 매수 ({current_price}){Colors.END}")
            trade_logger.info(f"🟢 진입 : 매수 | price: {current_price} | {return_position_msg}")
            ...
            active_logger.info(f"{Colors.RED}🔴 진입 : 매도 ({current_price}){Colors.END}")
            trade_logger.info(f"🔴 진입 : 매도 | price: {current_price} | {return_position_msg}")
            return_position = "short"
            play_voice_alert("매도 신호 발생")
            #send_telegram(f"매도 신호 발생: {current_price} " + return_position_msg)
        else:
            return_position = ""
            active_logger.info(f"⚪ 진입/청산 신호 : 없음 | {return_position_msg}")
        ...
        return return_trend, return_position, return_close, return_position_msg
    ...
    def execute_transaction(self, action: str, close_position: str) -> tuple:
        ...
        # === 3. 포지션 상태 확인 ===
        (has_long, long_amount, long_price), (has_short, short_amount, short_price) \
            = self.position_manager.get_all_positions(self.exchange, self.symbol)

        # === 4. 청산신호 확인 및 실행 ===
        # auto_close_opposite 옵션 체크
        auto_close_opposite = self.config['trading'].get('auto_close_opposite', False)
        if not auto_close_opposite:
            active_logger.info("auto_close_opposite=False 설정으로 청산을 건너뜁니다.")
        else:
            if close_position is None:
                ...
                if action == 'long' and has_short:
                    ...
                    if success:
                        active_logger.info(f"{Colors.BLUE}✓ 'long' 포지션 청산 완료!{Colors.END}")
                elif action == 'short' and has_long:
                    success = self.position_manager.close_position(self.exchange, self.symbol, 'long', long_amount)
                    if success:
                        active_logger.info(f"{Colors.BLUE}✓ 'short' 포지션 청산 완료!{Colors.END}")
            else:
        ...
        if close_position is None:
            ...
            if analysis and analysis.get('has_signal'):
                ...
                split_amount = self.config['trading']['order_amount_usdt'] / self.config['trading']['entry_split_count']
                ...
                if entry_count >= self.config['trading']['entry_split_count']:
                    active_logger.info(f"{Colors.YELLOW}⚠️ 분할 진입 최대 횟수({self.config['trading']['entry_split_count']}) 초과 → 진입 거절{Colors.END}")
                    entry_limit_flag = True
                if total_amount + split_amount > self.config['trading']['order_amount_usdt']:
                    active_logger.info(f"{Colors.YELLOW}⚠️ 분할 진입 총액 초과: {total_amount + split_amount:.2f} > {self.config['trading']['order_amount_usdt']:.2f} USDT → 진입 거절{Colors.END}")
                    entry_limit_flag = True

                entry_limit_flag = True ## 임시로 진입 제한
                ...
                if entry_limit_flag is False:
                    ...
                    success = self.position_manager.execute_trade(
                        ...
                        analysis['sl_price'],
                        analysis['tp_price'],
                        amount_mode=self.config['trading'].get('amount_mode', 'notional'),
                ...
                active_logger.error(f"{Colors.RED}⚪ 진입가, 익절가, 손절가 생성 실패 ){Colors.END}")

        return success_count, skipped_count
    
    def determine_trend_signal(self, ohlcv, df: pd.DataFrame, ema_series: dict, ema_vals: dict, df_bb_up: pd.DataFrame, df_bb_ma: pd.DataFrame, df_bb_dn: pd.DataFrame, df_rsi:pd.DataFrame,  current_price: float, tf_fast, tf_medium) -> str:
        """
        ...
        try:
            ...
            if bull_divergence_low or bull_divergence_pre:
                ...
                #active_logger.info(f"[추세반전] RSI 강세 다이버전스: {bull_divergence_pre}, {bull_divergence_low}")
            ...
            if bear_divergence_low or bear_divergence_pre:
                ...
                #active_logger.info(f"[추세반전] RSI 약세 다이버전스: {bear_divergence_pre}, {bear_divergence_low}")
            ...
            up_conds = [
                ...
                (rsi_trend == "strong_up") and not (rsi_trend == "strong_down"),
            ...
            dn_conds = [
                ...
                (rsi_trend == "strong_down") and not (rsi_trend == "strong_up"),
            ...
            active_logger.info(
                ...
                f" RSI추세= {to_text((rsi_trend == 'uptrend') and not (rsi_trend == 'downtrend'))}, RSI강도= {to_text((rsi_trend == 'strong_up') and not (rsi_trend == 'strong_down'))}," \
            ...
            active_logger.info(
                ...
                f" RSI추세= {to_text((rsi_trend == 'downtrend') and not (rsi_trend == 'uptrend'))}, RSI강도= {to_text((rsi_trend == 'strong_down') and not (rsi_trend == 'strong_up'))}," \
            ...
            return return_trend, return_msg
    ...
    def determine_trade_signal(self, ohlcv, df: pd.DataFrame, ema_series: dict, ema_vals: dict, df_bb_up: pd.DataFrame, df_bb_ma: pd.DataFrame, df_bb_dn: pd.DataFrame, df_rsi:pd.DataFrame,  current_price: float, tf_fast, tf_medium) -> str:
        ...
        if band_position_lower: # 볼린저밴드 하단 위치 신호(상승)
            # RSI 강세 다이버전스 감지
            bull = technical_indicators.detect_rsi_bull_divergence_local(df_rsi, df['close'], lookback=20)
            if bull:
                return "long", f"RSI 강세 다이버전스(전저점)"
            bull_divergence = technical_indicators.detect_rsi_bull_divergence(df_rsi, df['close'], lookback=20)
            if bull_divergence: 
                return "long", f"RSI 강세 다이버전스(최근)"

            # 직전거래량 급증 확인
            if prev_volume_up:
                spike_result = technical_indicators.detect_spike_reversal(df)
                if spike_result.get('down_reversal'):
                    return "long", "⭐️ 스파이크 반전(Down Reversal) 패턴 감지로 매수"
        elif band_position_upper: # 볼린저밴드 상단 위 위치 신호(하락)
            # RSI 약세 다이버전스 감지
            bear = technical_indicators.detect_rsi_bear_divergence_local(df_rsi, df['close'], lookback=20)
            if bear:
                return "short", f"RSI 약세 다이버전스(전고점)"
            bear_divergence = technical_indicators.detect_rsi_bear_divergence(df_rsi, df['close'], lookback=20)
            if bear_divergence:
                return "short", f"RSI 약세 다이버전스(최근)"

            # 직전거래량 급증 확인
            if prev_volume_up: 
                spike_result = technical_indicators.detect_spike_reversal(df)
                if spike_result.get('up_reversal'):
                    return "short", "⭐️ 스파이크 반전(Up Reversal) 패턴 감지로 매도"
        ...
        if resistance_breakup and volume_breakup:
            if rsi_last >= 50 and is_upper_riding and current_price > df['open'].iloc[-1]:                  # 저항선 돌파 신호
                return "long", f"추세 지속 매수(저항선돌파: {resistance_breakup}, 거래량돌파: {volume_breakup})"
            elif rsi_last > 70 and current_price < df['high'].iloc[-1] and df['close'].iloc[-1] < df['open'].iloc[-1]: # 저항선 반전 신호
                return "short", f"⭐️ 추세 반전 매도 (저항선반전: {resistance_breakup}, 거래량반전: {volume_breakup})"
        elif support_breakdown and volume_breakdown:
            if rsi_last < 30 and current_price > df['open'].iloc[-1] and df['close'].iloc[-1] > df['open'].iloc[-1]: # 지지선 반전 신호
                return "long", f"⭐️ 추세 반전 매수 (지지선반전: {support_breakdown}, 거래량반전: {volume_breakdown})"
            elif rsi_last <= 50 and is_lower_riding and current_price < df['low'].iloc[-1]:                # 지지선 파괴 신호
                return "short", f"추세 지속 매도 (지지선파괴: {support_breakdown}, 거래량파괴: {volume_breakdown})"
            
        # 3.볼린저밴드 상단/하단 근접 여부 (예: 1% 이내)
        is_near_upper_band = abs(current_price - df_bb_up.iloc[-1]) / df_bb_up.iloc[-1] < 0.01
        is_near_lower_band = abs(current_price - df_bb_dn.iloc[-1]) / df_bb_dn.iloc[-1] < 0.01

        # 저항선/지지선 및 거래량 신호 처리 (상단/하단 모두 대응)
        if resistance_breakup and volume_breakup:
            if rsi_last >= 65 and is_lower_riding and is_near_upper_band and current_price <= df['close'].iloc[-1]:
            ...
            if rsi_last <= 35 and is_lower_riding and is_near_lower_band and current_price > df['close'].iloc[-1]:
                # 과매도+저점 돌파 후 매수 (반전 또는 저점 매수)
                return "long", f"과매도+저점 돌파 후 매수 (지지선반전: {support_breakdown}, 거래량반전: {volume_breakdown})"
            elif rsi_last > 35 and is_upper_riding and current_price < df['close'].iloc[-1]:
                # 정상적인 돌파 매도
                return "short", f"추세 반전 매도 (지지선반전: {support_breakdown}, 거래량반전: {volume_breakdown})"

        # 4.신호 결정 (일반조건: 밴드값, RSI, 거래량 기반)
        if return_position == None:
            if band_position_belowlow and rsi_last <= self.rsi_oversold:       # 과매도 구간(상승)
                return_position = "long"
                return_msg = "볼린저밴드 하단 아래 위치 및 RSI 과매도 상태로 매수"
            elif band_position_aboveupper and rsi_last >= self.rsi_overbought: # 과매수 구간(하락)
                return_position = "short"
                return_msg = "볼린저밴드 상단 위 위치 및 RSI 과매수 상태로 매도"
            if volume_breakup and rsi_last < 65 and is_upper_riding:                # 거래량 돌파 신호(상승)
                return_position = "long"
                return_msg = "거래량 돌파 신호로 매수"
            elif volume_breakdown and rsi_last > 35 and is_lower_riding:             # 거래량 파괴 신호(하락)
                return_position = "short"
                return_msg = "거래량 파괴 신호로 매도"                                
            elif prev_volume_up:                    # 직전거래량 급증 확인
                if df['close'].iloc[-1] > df['open'].iloc[-1] and df['close'].iloc[-1] < current_price:
                    return_position = "long"
                    return_msg = "거래량 급증 신호로 매수"
                elif df['close'].iloc[-1] < df['open'].iloc[-1] and df['close'].iloc[-1] > current_price:
                    return_position = "short"
                    return_msg = "거래량 급증 신호로 매도"

        # 5.신호 보정 (진입 제한 조건)
        if return_position == "long" or return_position == "short":
            # 비율 0.5 이하이면 최소 밴드폭 상태로 간주 
            # if bandwidth_stats['bandwidth_ratio'] <= 0.7:
            #         return "", f"밴드폭 수축 상태로 진입 신호 제한 | {return_msg}"

            # EMA 배열 상태 / RSI 과열 구간 판별
            ema_position = technical_indicators.get_ema_position(ema_series, fast=tf_fast, medium=tf_medium)
            rsi_last = df_rsi.iloc[-1] if hasattr(df_rsi, 'iloc') else df_rsi[-1]
            is_rsi_overbought = rsi_last >= self.rsi_overbought
            is_rsi_oversold = rsi_last <= self.rsi_oversold

            # EMA 정배열(상승) 시 매도 금지, 단 RSI 과매수+연속상승봉이 아닐 때만 예외 허용
            if return_position == "short" and ema_position == 'uptrend':
                if is_rsi_overbought and not is_upper_riding:
                    return "short", f"⭐️ RSI과매수+연속상승봉 아님 | {return_msg}"
            # EMA 역배열(하락) 시 매수 금지, 단 RSI 과매도+연속하락봉이 아닐 때만 예외 허용
            elif return_position == "long" and ema_position == 'downtrend':
                if is_rsi_oversold and not is_lower_riding:
                    return "long", f"⭐️ RSI과매도+연속하락봉 아님 | {return_msg}"

            # 연속 양봉/음봉 감지 : 상승 연속봉 일때 매도 제한 / 하락 연속봉 일때 매수 제한
            if return_position == "short" and is_upper_riding and band_position_upper:  
                return_position = ""
                return_msg = f"⭐️ 연속 양봉 시 매도 제한 | {return_msg}"
            elif return_position == "long" and is_lower_riding and band_position_lower:
                return_position = ""
                return_msg = f"⭐️ 연속 음봉 시 매수 제한 | {return_msg}"
    ...
    def detect_ema_cross(self, ind: dict, fast=None, medium=None) -> str:
        ...
        if ema_fast is None or ema_medium is None:
            logging.error(f"[detect_ema_cross] EMA dict에 키 없음: fast={fast}, medium={medium}, 사용가능키={list(ind.keys())}")
            return None
